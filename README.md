...
- `/mj dump-ui [name]` writes a stable UI-node JSON under `%APPDATA%/MahjongHelper/ui-dumps/` for offline classify. Copy a trimmed dump into `MahjongHelper.Tests/Fixtures/ui/` (see that folder's README). Includes allIconNodes / tileSizedNodes / populated trays, seat/round, and the current snap meld line as `expected`.
