...
- `/mj screenshot` (alias `/mj printscreen`) fires FFXIV’s built-in screenshot. Watch folder comes from `ScreenShotDir` in `FFXIV.cfg` (OneDrive and non-OneDrive `My Games\FINAL FANTASY XIV - A Realm Reborn`) when set, then the usual `Documents\My Games\...\screenshots` fallbacks; newest-file search scans **all** candidates. Priority: game `ScreenShot.ScheduleScreenShot` (async — chat reports `ScreenShotResult` / `ScreenShotLocation` or a stuck `ScreenShotRequested`, and does **not** also inject a key) → bound `KEY_SCREENSHOT` → emulate `VK_SNAPSHOT` / PrintScreen. If `ScreenShotRequested` stays true with a missing/stale Location, the plugin force-clears that bit and retries schedule once (relog also clears it). `Result=Success` with no file on disk is treated as failure (ReShade/DXGI often eats the capture). Key inject does **not** claim a written PNG and is used only after Requested is false and a fresh schedule fails; if the bind is PrintScreen, chat reminds you to rebind Screenshot in System Config (e.g. F12). `/mj screenshot status` dumps CanTake / Requested / Result / LocationOnDisk / resolved dirs. Telesto is not required; it can still `ExecuteCommand` `/mj screenshot`. Hide UI / Scroll Lock is **not** toggled.
