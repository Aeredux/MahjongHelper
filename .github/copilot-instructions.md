...
- **Build command**: Always build via the solution file: `dotnet build SamplePlugin.sln -c Release`. This uses the `Release|x64` platform defined in the solution and outputs to `SamplePlugin/bin/x64/Release/SamplePlugin.dll`. Do NOT use `-r win-x64` on the csproj directly — that outputs to a different path (`bin/Release/win-x64/`) which is not the correct deployment artifact.
